...
# CPU Utilization Statistics Unix Platform ( Linux,AIX,Solaris are supported )
case `uname` in
  	Linux ) CPU_REPORT=`vmstat -n $INTERVAL_SEC $NUM_REPORT | tail -1`

            if [ "${CPU_REPORT}" == "" ]; then
              read CPU_USER CPU_SYSTEM CPU_IOWAIT CPU_IDLE <<< "0 0 0 0"
            else
              read CPU_USER CPU_SYSTEM CPU_IOWAIT CPU_IDLE <<< $( echo $CPU_REPORT | awk '
              BEGIN{
                USER=0
                SYS=0
                WAIT=0
